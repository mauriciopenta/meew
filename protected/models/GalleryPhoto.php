...
class GalleryPhoto extends CActiveRecord
{
    ...
    public function rules()
	{
		...
		return array(
			...
			array('gallery_id, rank, precio, unidades, tipo_contenido', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>512),
			array('file_name', 'length', 'max'=>128),
			array('precio_txt', 'length', 'max'=>45),
			array('description, url_video', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, gallery_id, rank, name, description, file_name, precio, precio_txt, unidades, url_video, tipo_contenido', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function search()
	{
		...
		$criteria->compare('precio_txt',$this->precio_txt,true);
