...
class GalleryBehavior extends CActiveRecordBehavior
{
    ...
    /** @var boolean does images in gallery need precio_txt */
    public $precio_text=false;
    /** @var boolean does images in gallery need precio */
    public $precio=false;
    /** @var boolean does images in gallery need unidades */
    public $unidades=false;
    /** @var boolean does images in gallery need url_video */
    public $url_video=false;
    ...
    public function beforeSave($event)
    {
        ...
        if ($event->isValid) {
            if (empty($this->getOwner()->{$this->idAttribute})) {
                ...
                $gallery->url_video = $this->url_video;
                $gallery->precio_text = $this->precio_text;
                $gallery->precio = $this->precio;
                $gallery->unidades = $this->unidades;
    ...
    public function changeConfig($force = false)
    {
        ...
        if ($gallery->versions_data != serialize($this->versions) || $force) {
            ...
            $gallery->precio_txt = $this->precio_txt;
